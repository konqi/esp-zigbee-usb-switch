...
esp_err_t toggle_driver_gpio_init(uint8_t gpio_pin)
{
    ...
    ESP_RETURN_ON_ERROR(gpio_set_level(gpio_pin, 1), TAG, "Unable to set gpio value for pin %i", gpio_pin);
...
static void oneshot_timer_callback(void *arg)
{
    ...
    ESP_ERROR_CHECK(gpio_set_level(gpio_pin, 1));
}

esp_err_t schedule_off(uint8_t gpio_pin)
{
    ...
    /* Start the timers */
    return esp_timer_start_once(oneshot_timer, 1 * 500000);
}

esp_err_t toggle_gpio(uint8_t gpio_pin, uint16_t durationTicksMs)
{
    ESP_RETURN_ON_ERROR(gpio_set_level(gpio_pin, 0), TAG, "Unable to set gpio value for pin %i", gpio_pin);
    ESP_RETURN_ON_ERROR(schedule_off(gpio_pin), TAG, "Cannot schedule gpio toggle reversal");
